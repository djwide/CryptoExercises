...
with open('RAWAES2.txt') as input_file:
    ciphertextRaw= input_file.read()
a=()
maxReps=0
for x in range(1, 205):
    thisRoundReps= count_repetitions(ciphertextRaw[0:320*x], 16, x)["repetitions"]
    if(thisRoundReps>maxReps):
        maxReps= thisRoundReps
        maxInd= count_repetitions(ciphertextRaw[0:320*x], 16, x)["line number"]
print(maxReps)
print(maxInd)
